...
barcode = \data, b, { dpi ? D150 }, { fw ? O0 } ->
    Str.concat
        ...
        (
            when b is
                Aztec { orientation ? None, scale ? None, eci ? "N", size ? 0, readerInit ? "N", symbols ? 1, id ? 0 } ->
                    o =
                        enum
                            (
                                when orientation is
                                    None -> Orientation fw # (read s Fw)
                                    Some a -> Orientation a
                            )
                    m =
                        Num.toStr
                            (
                                when scale is
                                    None -> spec (MaxDpi dpi)
                                    Some a -> a
                            )
                    Str.concat
                        "0"
                        (
                            [o, m, eci, Num.toStr size, readerInit, Num.toStr symbols, Num.toStr id]
                            |> Str.joinWith ","
                        )
...
expect
    actual = barcode " 7. This is testing label 7" (Aztec { orientation: Some O90, scale: Some 7 }) {} {}
...
expect
    ...
    actual = graphic (Field { format: Ascii, dataBytes: 5000, totalBytes: 5000, rowBytes: 25, data: image })
...
create = \elem ->
    when elem is
        ...
        Text opt ->
            ...
            "$(enum (Font fontSize))$(enum Text)$(content)"

        Picture opt ->
            { b, c, d, base64 } = opt
            "^GFA,$(Num.toStr b),$(Num.toStr c),$(Num.toStr d),$(base64)"
...
expect
    actual = build [
        ...
        ((250, 50), Picture { b: 8000, c: 8000, d: 80, base64: "data:image/png;base64" }),
    ]
