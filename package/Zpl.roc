...
Rotation : [R0, R180, R270, R90]
Color : [Black, White]

Barcode : [
    Aztec { eci ? Str, id ? U8, readerInit ? Str, rotation ? Rotation, scale ? U8, size ? U8, symbols ? U8 },
    Code11 { checkDigit : Str, height : U8, line : Str, lineAbove : Str, rotation ? Rotation },
    Code39 { checkDigit : Str, height : U8, line : Str, lineAbove : Str, rotation ? Rotation },
    Interleaved2Of5 { checkDigit : Str, height : U8, line : Str, lineAbove : Str, rotation ? Rotation },
    QrCode { dataInput ? [Automatic, Manual [Alphanumeric, Numeric]], errorCorrection ? [HighDensity, HighReliability, Standard, UltraHighReliability], model ? U8, scale : U8 },
]

Graphic : [
    Box { color ? Color, height ? U8, rounding ? U8, thickness ? U8, width ? U8 },
    Circle { color ? Color, thickness : U8, width : U8 },
    DiagonalLine { color ? Color, height : U8, leaning ? [Left, Right], thickness : U8, width : U8 },
    Ellipse { color ? Color, height : U8, thickness : U8, width : U8 },
    Field { data : Str, dataBytes : U8, format ? [Ascii, Binary, Compressed], rowBytes : U8, totalBytes : U8 },
    Symbol { char : [CanadianStandardsAssociationApproval, Copyright, RegisteredTradeMark, TradeMark, UnderwritersLaboratoriesApproval], rotation ? Rotation },
]
...
toZpl = \flag ->
    when flag is
        FormatStart -> "^XA"
        FormatEnd -> "^XZ"
        FieldOrigin { top ? 0, left ? 0 } -> "^FO$(Num.toStr top),$(Num.toStr left)"
        FieldSeparator -> "^FS"
        FieldReverse -> "^FR"
        FieldData data -> "^FD" |> Str.concat data
        Font { name ? "0", rotation ? R0, height, width } -> Str.concat "^A$(name)" ([toZpl (Rotation rotation), Num.toStr height, Num.toStr width] |> Str.joinWith ",")
        Color t ->
            when t is
                Black -> "B"
                White -> "W"
        ...
        (
            when type is
                ...
                Aztec { rotation ? fw, scale ? spec (MaxDpi dpi), eci ? "N", size ? 0, readerInit ? "N", symbols ? 1, id ? 0 } ->
                    Str.concat "0" ([toZpl (Rotation rotation), Num.toStr scale, eci, Num.toStr size, readerInit, Num.toStr symbols, Num.toStr id] |> Str.joinWith ",")
                    |> Str.concat (toZpl (FieldData data))

                Code11 { rotation ? fw, checkDigit, height, line, lineAbove } ->
                    Str.concat "1" ([toZpl (Rotation rotation), checkDigit, Num.toStr height, line, lineAbove] |> Str.joinWith ",")
                    |> Str.concat (toZpl (FieldData data))

                Interleaved2Of5 { rotation ? fw, height, line, lineAbove, checkDigit } ->
                    Str.concat "2" ([toZpl (Rotation rotation), Num.toStr height, line, lineAbove, checkDigit] |> Str.joinWith ",")
                    |> Str.concat (toZpl (FieldData data))

                Code39 { rotation ? fw, checkDigit, height, line, lineAbove } ->
                    Str.concat "3" ([toZpl (Rotation rotation), checkDigit, Num.toStr height, line, lineAbove] |> Str.joinWith ",")
                    |> Str.concat (toZpl (FieldData data))
                ...
                QrCode { model ? 2, scale, errorCorrection ? Standard, dataInput ? Automatic } ->
                    ...
                    |> Str.concat (toZpl (FieldData ([ec, di, data] |> Str.joinWith "")))
        )

expect barcode " 7. This is testing label 7" (Aztec { rotation: R90, scale: 7 }) {} == "^B0R,7,N,0,N,1,0^FD 7. This is testing label 7"
expect barcode "123456" (Code11 { checkDigit: "N", height: 150, line: "Y", lineAbove: "N" }) {} == "^B1N,N,150,Y,N^FD123456"
expect barcode "123456" (Interleaved2Of5 { checkDigit: "N", height: 150, line: "Y", lineAbove: "N" }) {} == "^B2N,150,Y,N,N^FD123456"
expect barcode "123456" (Code39 { checkDigit: "N", height: 150, line: "Y", lineAbove: "N" }) {} == "^B3N,N,150,Y,N^FD123456"
expect barcode "AC-42" (QrCode { scale: 10, dataInput: Manual Alphanumeric }) {} == "^BQ,2,10^FDMM,AAC-42"
        ...
        (
            when type is
                Box { width ? 1, height ? 1, thickness ? 1, color ? Black, rounding ? 0 } ->
                    ...
                    Str.concat "B" ([Num.toStr width, Num.toStr height, Num.toStr thickness, toZpl (Color color), Num.toStr rounding] |> Str.joinWith ",")

                Circle { width, thickness, color ? Black } ->
                    # TODO: clamp numbers like width 3 to 4095 as spec
                    Str.concat "C" ([Num.toStr width, Num.toStr thickness, toZpl (Color color)] |> Str.joinWith ",")

                Ellipse { width, height, thickness, color ? Black } ->
                    Str.concat "E" ([Num.toStr width, Num.toStr height, Num.toStr thickness, toZpl (Color color)] |> Str.joinWith ",")
                ...
                DiagonalLine { width, height, thickness, color ? Black, leaning ? Right } ->
                    l =
                        when leaning is
                            Right -> "R"
                            Left -> "L"
                    Str.concat "D" ([Num.toStr width, Num.toStr height, Num.toStr thickness, toZpl (Color color), l] |> Str.joinWith ",")
                ...
                Symbol { rotation ? R0, char } ->
                    ...
                    Str.concat "S" ([toZpl (Rotation rotation)] |> Str.joinWith (","))
                    |> Str.concat (toZpl (FieldData d))
        )

expect graphic (Box { width: 200, height: 100, thickness: 8, rounding: 0 }) == "^GB200,100,8,B,0"
expect graphic (Circle { width: 100, thickness: 4 }) == "^GC100,4,B"
expect graphic (DiagonalLine { width: 150, height: 50, thickness: 2, leaning: Right }) == "^GD150,50,2,B,R"
expect graphic (Ellipse { width: 120, height: 60, thickness: 3 }) == "^GE120,60,3,B"
expect
    ...
    actual == expected
expect graphic (Symbol { char: TradeMark }) == "^GSN^FDC"

create = \elem ->
    when elem is
        Graphic x -> graphic x
        Barcode (x, y) -> barcode x y {}
        Text { fontSize, content } -> "$(toZpl (Font fontSize))$(toZpl (FieldData content))"
        Field x -> field x

field = \((top, left), children) ->
    "$(toZpl (FieldOrigin { top, left }))$(create children)$(toZpl FieldSeparator)"

build = \list ->
    list
    |> List.map field
    |> List.prepend (toZpl FormatStart)
    |> List.append (toZpl FormatEnd)
    |> Str.joinWith ""

expect
    actual = build [
        ((50, 50), Graphic (Box { width: 100, height: 50, thickness: 2 })),
        ((100, 100), Text { fontSize: { height: 30, width: 30 }, content: "Hello zpl from roc!" }),
